...
- [ ] Add `--with github.com/mholt/caddy-ratelimit@v0.1.0`
...
- [ ] Declare the required module set in the runtime, next to the code that emits module ids
- [ ] Add the Rust test: build an everything-on `ProxyConfig`, walk the emitted JSON for module ids, assert each is declared. Keep the fixture maximal, and say so where it lives, because a feature the fixture does not exercise contributes no module id
