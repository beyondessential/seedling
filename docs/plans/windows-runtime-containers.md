...
- **S3. Networking on a worst-case image** (field disk image) — per-instance compartment and endpoint; service addresses; mount graph compiled to compartment-boundary enforcement with default-deny; ingress host public-port publishing; per-compartment resolver DNS; coexistence with field AV/EDR. Also whether a workload holding administrative rights inside its container can alter its own compartment's policy — `wcr[net.dataplane]` claims it cannot, and that claim is load-bearing for the threat model. Decides Q3.
