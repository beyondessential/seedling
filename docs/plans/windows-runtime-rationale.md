...
Input for the Windows threat-model document: the daemon–supervisor pipe is authenticated by ACL plus server-identity verification; an elevated process can still impersonate, consistent with the discretionary-enforcement posture of `win[wfp.honesty]`. Service installation fires event 4697 per registration — including per dynamic-Job invocation under the chosen design — and is a persistence technique EDR watches; service deletion is not distinctly audited, so SIEMs observe creations without matching removals. No invisibility is claimed. There is no credential surface: nothing has a password, nothing appears in account-review reports.
