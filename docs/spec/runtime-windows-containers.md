...
> wc[net.dns]
> Each compartment resolves the Seedling zone through the [resolver](#wc--infra.resolver) (`r[infra.pod.dns]`); other resolution follows the host's configuration.
...
> wc[stop.ladder]
> The stop sequence delivers the declared [stop method](#wc--stop.methods), waits `stop_timeout` (`l[container.stop-signal]`, `l[container.stop-timeout]`), then terminates the container.
...
> wc[signal.exit-code]
> When the runtime terminates a process — the [stop ladder](#wc--stop.ladder)'s final step, a signal-mapped termination, or a session teardown — it records a negative exit code, so `i[shell.exit]`'s convention distinguishes runtime termination from the process's own exit code.
