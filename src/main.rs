...
mod tests {
    ...
    fn uncaught_error_fails_script() {
        ...
        let result = run_script(&engine, &mut scope, r#"throw "deliberate failure";"#);
    ...
    fn port_validation_rejects_invalid() {
        let (engine, mut scope, _app) = setup();
        let result = run_script(&engine, &mut scope, r#"app.service("web").port(0);"#);
        assert!(result.is_err(), "port 0 must be rejected");

        let (engine, mut scope, _app) = setup();
        let result = run_script(&engine, &mut scope, r#"app.service("web").port(65535);"#);
        assert!(result.is_err(), "port 65535 must be rejected");

        let (engine, mut scope, _app) = setup();
        let result = run_script(&engine, &mut scope, r#"app.service("web").port(-1);"#);
        assert!(result.is_err(), "negative port must be rejected");
    }
    ...
    #[test]
    fn resource_types_are_collections() {
        let app = run_test_script_app(
            r#"
            let svc = app.service("web");
