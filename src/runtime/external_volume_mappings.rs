...
pub fn create(db: &Db, mapping: &ExternalVolumeMapping) -> rusqlite::Result<()> {
    ...
    db.conn.execute(
        "INSERT INTO external_volume_mappings (app, external_name, target_kind, target_app, target_volume)
         VALUES (?1, ?2, ?3, ?4, ?5)",
        params![mapping.app, mapping.external_name, kind, target_app, target_volume],
    )?;
    Ok(())
}
...
pub fn update(db: &Db, mapping: &ExternalVolumeMapping) -> rusqlite::Result<bool> {
    ...
    let count = db.conn.execute(
        "UPDATE external_volume_mappings
         SET target_kind = ?3, target_app = ?4, target_volume = ?5
...
pub fn list_for_app(db: &Db, app: &str) -> rusqlite::Result<Vec<ExternalVolumeMapping>> {
    let mut stmt = db.conn.prepare(
        "SELECT app, external_name, target_kind, target_app, target_volume
