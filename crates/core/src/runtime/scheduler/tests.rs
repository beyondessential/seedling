...
fn empty_scheduler_accepts_first_request() {
    ...
    assert_eq!(
        s.request(&app("app1"), "start", serde_json::Map::new(), 1, 1, "operator"),
...
fn same_app_different_action_still_rejected() {
    ...
    s.request(&app("app1"), "start", serde_json::Map::new(), 1, 1, "operator");
    assert_eq!(
        s.request(&app("app1"), "deploy", serde_json::Map::new(), 1, 1, "operator"),
        ScheduleResult::Rejected(RejectReason::SameAppOperationInProgress)
    );
}
...
fn different_app_request_is_queued() {
    ...
    s.request(&app("app1"), "start", serde_json::Map::new(), 1, 1, "operator");
    assert_eq!(
        s.request(&app("app2"), "start", serde_json::Map::new(), 1, 1, "operator"),
...
fn already_queued_app_is_rejected() {
    let mut s = Scheduler::new();
    s.request(&app("app1"), "start", serde_json::Map::new(), 1, 1, "operator");
    s.request(&app("app2"), "start", serde_json::Map::new(), 1, 1, "operator"); // queued
    assert_eq!(
        s.request(&app("app2"), "start", serde_json::Map::new(), 1, 1, "operator"),
...
fn two_different_apps_can_both_queue() {
    ...
    assert_eq!(
        s.request(&app("app3"), "start", serde_json::Map::new(), 1, 1, "operator"),
        ScheduleResult::Queued
    );
...
fn queued_app_rejected_regardless_of_action_name() {
    ...
    s.request(&app("app2"), "start", serde_json::Map::new(), 1, 1, "operator");
    assert_eq!(
        s.request(&app("app2"), "deploy", serde_json::Map::new(), 1, 1, "operator"),
        ScheduleResult::Rejected(RejectReason::SameAppAlreadyQueued)
    );
}
